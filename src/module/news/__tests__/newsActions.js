...
describe('News', () => {
  describe('News Request', () => {
    it('Should return proper action', () => {
      const data = {
        limit: 1
      };
      const action = fetchNewsRequest(data);
  ...
  describe('News Success', () => {
    it('Should return proper action', () => {
      const action = fetchNewsSuccess(newsData);
      expect(action).toEqual({
        type: NEWS_SUCCESS,
        payload: newsData
      });
  ...
  describe('News Failure', () => {
    it('Should return proper action', () => {
      const action = fetchNewsFailure();
      expect(action).toEqual({ type: NEWS_FAILURE });
    });
  });
});
